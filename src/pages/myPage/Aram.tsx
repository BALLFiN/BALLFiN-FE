        ...
        <div className="p-6">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 bg-gray-50 rounded-lg p-6">
              <h3 className="text-sm font-medium text-gray-700 mb-4">새로운 알림 추가</h3>
              <div className="flex items-center gap-3">
                ...
                <select
                  value={selectedType}
                  onChange={(e) => setSelectedType(e.target.value as 'push' | 'email')}
            ...
            <div className="flex-1">
              <h3 className="text-sm font-medium text-gray-700 mb-4">설정된 알림 목록</h3>
              <div className="space-y-4">
                {alarms.map((alarm) => (
                  <div key={alarm.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      <Bell className={`w-5 h-5 ${alarm.isActive ? 'text-[#0A5C2B]' : 'text-gray-400'}`} />
                      <span className="font-medium text-gray-900">{alarm.keyword}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <select
                        value={alarm.notificationType}
                        onChange={(e) => handleTypeChange(alarm.id, e.target.value as 'push' | 'email')}
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-lg ${
                          alarm.isActive ? 'bg-[#0A5C2B]/10 text-[#0A5C2B]' : 'bg-gray-200 text-gray-600'
                        }`}
                      >
                        {alarm.isActive ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}
                      </button>
                      <button
