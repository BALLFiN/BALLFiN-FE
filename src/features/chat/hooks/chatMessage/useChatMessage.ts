...
export const useSendMessage = () => {
  ...
  return useMutation({
    ...
    onMutate: async ({ chatId, message }) => {
      ...
      const previousMessages = queryClient.getQueryData<NewMessagePT[]>(
        CHAT_KEYS.detail(chatId),
      );

      queryClient.setQueryData<NewMessagePT[]>(
        CHAT_KEYS.detail(chatId),
        (old) => [
          ...(old || []),
          {
    ...
    onError: (_err, _newMsg, context) => {
      if (context?.previousMessages && context?.chatId) {
        queryClient.setQueryData(
          CHAT_KEYS.detail(context.chatId),
