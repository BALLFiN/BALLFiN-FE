...
interface StockChartProps {
  ...
  timeRange: '1m' | '5m' | '15m' | '30m' | '1h' | '1d' | '1w' | '1m' | '3m' | '1y';
  onTimeRangeChange: (range: '1m' | '5m' | '15m' | '30m' | '1h' | '1d' | '1w' | '1m' | '3m' | '1y') => void;
      ...
      <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg">
        ...
        <div className="space-y-1 mt-2">
          <p className="text-sm">
            ...
            <span className="ml-2 font-medium">{data.open?.toLocaleString()}원</span>
          </p>
          <p className="text-sm">
            <span className="text-gray-600">고가:</span>
            <span className="ml-2 font-medium text-red-500">{data.high?.toLocaleString()}원</span>
          </p>
          <p className="text-sm">
            <span className="text-gray-600">저가:</span>
            <span className="ml-2 font-medium text-blue-500">{data.low?.toLocaleString()}원</span>
          </p>
          <p className="text-sm">
            <span className="text-gray-600">종가:</span>
            <span className="ml-2 font-medium">{data.close?.toLocaleString()}원</span>
          </p>
          <p className="text-sm">
          ...
          {data.ma5 && (
            <div className="border-t pt-2 mt-2">
              <p className="text-sm">
                ...
                <span className="ml-2 font-medium text-purple-500">{data.ma5?.toLocaleString()}원</span>
              </p>
              <p className="text-sm">
                <span className="text-gray-600">MA20:</span>
                <span className="ml-2 font-medium text-green-500">{data.ma20?.toLocaleString()}원</span>
              </p>
              <p className="text-sm">
                <span className="text-gray-600">MA60:</span>
                <span className="ml-2 font-medium text-orange-500">{data.ma60?.toLocaleString()}원</span>
              </p>
              <p className="text-sm">
                <span className="text-gray-600">MA120:</span>
                <span className="ml-2 font-medium text-blue-500">{data.ma120?.toLocaleString()}원</span>
              </p>
            </div>
          )}
        </div>
    ...
    <div className="h-[300px] mb-4">
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          ...
          {/* 주가 꺾은선 (빨간색) */}
          <Line type="monotone" dataKey="close" stroke="#ef4444" strokeWidth={2} dot={false} name="주가" />

          {/* 이동평균선들 */}
          {showMA.ma5 && (
            <Line type="monotone" dataKey="ma5" stroke="#8b5cf6" dot={false} strokeWidth={1.5} name="MA5" />
          )}
          {showMA.ma20 && (
            <Line type="monotone" dataKey="ma20" stroke="#22c55e" dot={false} strokeWidth={1.5} name="MA20" />
          )}
          {showMA.ma60 && (
            <Line type="monotone" dataKey="ma60" stroke="#f97316" dot={false} strokeWidth={1.5} name="MA60" />
          )}
          {showMA.ma120 && (
            <Line type="monotone" dataKey="ma120" stroke="#3b82f6" dot={false} strokeWidth={1.5} name="MA120" />
          )}
        </ComposedChart>
      </ResponsiveContainer>
    </div>
    ...
    <div className="h-[150px]">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 5, right: 30, left: 0, bottom: 0 }}>
              ...
              if (active && payload && payload.length) {
                return (
                  <div className="bg-white p-2 border border-gray-200 rounded shadow">
                    ...
                    <p className="text-sm text-gray-600">거래량: {payload[0].value?.toLocaleString()}</p>
                  </div>
                );
              }
              return null;
          ...
          />
          <Bar dataKey="volume" fill="#26a69a" opacity={0.7} radius={[2, 2, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
export default function StockChart({ data, showMA, timeRange, onTimeRangeChange, onToggleMA }: StockChartProps) {
  return (
